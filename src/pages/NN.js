...
const NN = () => {
    ...
    const project_detail = {
        title: productname,
        title_bar: (<p>Nocturnal<br />Wanderings</p>),
        bg: "",
        log: ImagesRoute.NNlog,
        top_bg: ImagesRoute.IMG_6728,
        project_state: (<a className=" bg-[#777777] px-[10px] py-[5px] rounded-[10px]" href="../assets/Dawn-NacturnalWanderings.zip" download>ダウンロード</a>),
        ...
        width_explain: (
            <p className={`scroll-section ${animationStyle} string-ss space-y-[10px]`}>
                <p className={`scroll-section ${animationStyle}`}>Nocturnal Wanderingsは、夜間作業を</p>
                <p className={`scroll-section ${animationStyle}`}>快適にサポートするフィルターソフトです。</p>
                <p className={`scroll-section ${animationStyle}`}>暗い部屋での作業中、画面の明るさが気になることはありませんか？</p>
                <p className={`scroll-section ${animationStyle}`}>このソフトは、PCの設定以上に明るさを抑えることで、</p>
                <p className={`scroll-section ${animationStyle}`}>目への負担を軽減しつつ集中力を高めます。</p><br /></p>
        ),
        explain2_src: ImagesRoute.gp83,
        explain2_title: "ディスプレイも夜仕様に",
        explain2_text: (<p className="string-ss space-y-[10px]">暗い部屋でも目に優しく、<br />
            快適に作業を続けられます。<br />
            Nocturnal Wanderingsを実行して、<br />
            夜間の作業をもっと快適に。<br /></p>),
        ...
        dialog: (
            <div>
                ...
                <div className="text-white leading-relaxed space-y-6">
                    <p>
                        このソフトの名前は「夜を行く人々」にしようと思っていましたが、<br />
                        調べていくうちに少しカッコつけた「Nocturnal Wanderings」に落ち着きました。
                    </p>
                    <h2 className="string-rg">主な機能</h2>
                    <p>
                        OSの設定限界よりも画面を暗くする機能<br />
                        ブルーライトをカットする機能
                    </p>
                    <h2 className="string-rg">開発の背景</h2>
                    <p>
                        私は夜になると部屋を暗くし、デスクライトだけで作業することが多いです。<br />
                        その理由は、夜に強い光を浴びると睡眠の質が低下するという話を聞いたことがあるのと、<br />
                        単純に暗い空間の雰囲気が好きだからです。<br />
                        そんな環境でパソコンを使っていると、白い画面がどうしても眩しく、<br />
                        明るさを最小に設定しても目に負担を感じました。<br />
                        そこで「フィルターウィンドウを作って画面に重ねれば暗くできるのでは？」と思い立ち、開発を始めました。<br />
                        ちょうどその頃、Microsoftが提供するC#のフレームワーク「.NET Framework」に興味があり、<br />
                        これを使って開発することにしました。
                    </p>
                    <h2 className="string-rg">追加機能へのアイデア</h2>
                    <p>
                        開発中、セカンドディスプレイを購入して使ってみたところ、<br />
                        明るさを調整する際に本体のボタンを押さなければならず不便さを感じました。<br />
                ...
                <p className="text-white">
                    フロントエンド: XAML,.NET Framework<br />
                    バックエンド:C#,.NET Framework<br />
                    デザイン: Adobe XD<br />
                    その他:
                </p>
            </div>
